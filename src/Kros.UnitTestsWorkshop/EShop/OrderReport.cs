...
public class OrderReport : IDocument
{
    ...
    void ComposeHeader(IContainer container)
    {
        ...
        container.Row(row =>
        {
            ...
            {
                column.Item().Text($"Order #{Model.Order.Number}").Style(titleStyle);

                column.Item().Text(text =>
                {
                    text.Span("Issue date: ").SemiBold();
                    text.Span($"{Model.Order.OrderDate:d}");
                });
            });
    ...
    void ComposeTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            {
                table.Cell().Element(CellStyle).Text((Model.Order.Items.IndexOf(item) + 1).ToString());
                ...
                table.Cell().Element(CellStyle).AlignRight().Text($"{item.TotalPrice}eur");
